...
ALL_STATUSES = AUTOMATIC_STATUSES + ("shared", "notified", "unknown", "manual")
...
class Outbox:
    ...
    def is_initialized(self) -> bool:
        row = self._conn.execute(
            "SELECT value FROM meta WHERE key = 'feed_initialized'"
        ).fetchone()
    ...
    def initialize_with_items(self, items: Sequence[FeedArticle], now: float) -> int:
        ...
        with self._conn:
            ...
            self._conn.execute(
                "INSERT INTO meta(key, value) VALUES ('feed_initialized', '1') "
    ...
    def _insert_items(self, items: Sequence[FeedArticle], now: float) -> int:
        before = self._conn.total_changes
        for item in items:
            self._conn.execute(
                """
                INSERT OR IGNORE INTO articles (
                    article_key, article_id, normalized_url, source_url,
    ...
    def has_eligible_work(self, now: float) -> bool:
        placeholders = ",".join("?" for _ in AUTOMATIC_STATUSES)
        row = self._conn.execute(
            f"SELECT 1 FROM articles WHERE status IN ({placeholders}) "
            "AND next_attempt_at <= ? LIMIT 1",
            (*AUTOMATIC_STATUSES, now),
        ).fetchone()
    ...
    def mark_fetched(self, article_key: str, article: FetchedArticle, now: float) -> None:
        self._update(
            """
            ...
            WHERE article_key=? AND status='fetch_pending'
            """,
            (
    ...
    def pending_shared(self, now: float) -> list[dict]:
        rows = self._conn.execute(
            ...
            WHERE status='shared' AND next_attempt_at <= ?
                AND (
                    message_uuid=''
                    OR NOT EXISTS (
    ...
    def reconcile_partial_message_groups(self, now: float) -> None:
        ...
        self._update(
            ...
            WHERE status='shared' AND message_uuid!=''
                AND EXISTS (
                    SELECT 1 FROM articles AS peer
    ...
    def activate_circuit(self, kind: str, now: float, duration: float) -> None:
        self._update(
            """
            INSERT INTO circuit(singleton, kind, until_at, activated_at)
            VALUES (1, ?, ?, ?)
            ON CONFLICT(singleton) DO UPDATE SET
                kind=excluded.kind,
                until_at=MAX(circuit.until_at, excluded.until_at),
                activated_at=excluded.activated_at
            """,
            (kind[:100], now + duration, now),
        )

    def circuit(self) -> Optional[dict]:
        row = self._conn.execute("SELECT * FROM circuit WHERE singleton=1").fetchone()
        return dict(row) if row else None
    ...
    def activate_alert(self, kind: str, now: float, cooldown: float) -> bool:
        row = self._conn.execute("SELECT * FROM alerts WHERE kind=?", (kind,)).fetchone()
        with self._conn:
            if row is None:
                self._conn.execute(
    ...
    def active_alerts(self) -> list[dict]:
        rows = self._conn.execute(
            "SELECT * FROM alerts WHERE active=1 ORDER BY started_at"
        ).fetchall()
        return [dict(row) for row in rows]
